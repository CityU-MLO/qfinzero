...
def build_standardized_export(
    ...
    export_data = {
        ...
        "configuration": {
            ...
            "methods": configuration.get("methods", ["baseline", "lasso", "ic_optimization"]),
        ...
        "results": {
            ...
            "successful_periods": sum(1 for p in optimization_results.get("periods", []) 
        ...
        export_data["configuration"]["period_config"]["dynamic_periods"] = [
            {
                ...
                "test_end": p.get("test_end") or p.get("end"),
                "training": {
                    "type": "global_lookback" if optimization_type == "dynamic" else "fixed",
    ...
    export_data["results"]["success_rate"] = (successful / total * 100) if total > 0 else 0
...
def factor_combination_dynamic():
    ...
    try:
        ...
        if not periods_config or not isinstance(periods_config, list):
            # Debug info
            logger = logging.getLogger(__name__)
            logger.error(f"Invalid periods_config: type={type(periods_config)}, value={periods_config}")
        ...
        for i, period in enumerate(periods_config):
            ...
            if 'start' in period and 'end' in period:
                ...
                if ui_train_start and ui_train_end:
                    ...
                    train_start_dt = train_end_dt - timedelta(days=365) # Default 1 year lookback
                    
                    transformed_period['train_start'] = train_start_dt.strftime('%Y-%m-%d')
                    transformed_period['train_end'] = train_end_dt.strftime('%Y-%m-%d')
            
            elif 'train_start' in period and 'train_end' in period and 'test_start' in period and 'test_end' in period:
        ...
        def validate_dynamic_continuity(periods):
            ...
            for i in range(len(sorted_periods) - 1):
                try:
                    from datetime import datetime, timedelta
                    current_end = datetime.strptime(sorted_periods[i]['test_end'], '%Y-%m-%d')
                    next_start = datetime.strptime(sorted_periods[i+1]['test_start'], '%Y-%m-%d')
                    
                    gap_days = (next_start - current_end).days
                    
                    # Ideally should be 1 day gap (next trading day) or account for weekends
                    if gap_days < 1:
                        issues.append(f"Periods {i+1}-{i+2}: Overlap in testing periods")
                    elif gap_days > 3:
                        warnings.append(f"Periods {i+1}-{i+2}: {gap_days} day gap in testing periods (may skip weekends/holidays)")
        ...
        if 'ic_optimization' in methods:
            method_configs['ic_optimization'] = {
                'method': 'equal_top',
                'top_k': 5
            }
        ...
        export_format = build_standardized_export(
            ...
            factors=[{"formula": f} for f in (periods_config[0].get("factors", []) if periods_config else [])],
            periods_config=transformed_periods,
        ...
        for period_result in result_serializable.get('periods', []):
            ...
            for method in ['lasso', 'ic_optimization', 'baseline']:
                if period_result.get(method) and period_result[method].get('status') == 'success':
...
def factor_combination_fixed():
    ...
    try:
        ...
        if 'periods' in data and isinstance(data['periods'], list) and len(data['periods']) > 0:
            ...
            print(f"[CATEGORY A] Using periods array format - first period: {training_start} to {training_end}, {len(factors)} factors")
        else:
            # Original format
        ...
        if not training_start or not training_end or not testing_start or not testing_end:
            return jsonify({
                "success": False,
        ...
        if not factors or not isinstance(factors, list):
            return jsonify({
                "success": False,
                "message": "factors must be a non-empty list"
            })
        ...
        period_results = result_serializable.get('periods', [{}])[0] if result_serializable.get('periods') else {}
        logger.info(f"Extracted period results: {period_results}")
        
        # Add testing period info
        period_results['testing_period'] = {
            'start': testing_start,
            'end': testing_end
        }
...
def factor_combination_review():
    ...
    try:
        ...
        if action == 'load':
            ...
            try:
                results, configuration = UnifiedOptimizationPipeline.import_results(filepath)
        ...
        # Action: Validate periods continuity
            ...
            validation = UnifiedOptimizationPipeline.validate_periods_continuity(periods_config)
...
def run_backtest_from_weights():
    ...
    try:
        ...
        if not weights_data:
            return jsonify({
                "success": False,
                "message": "weights_data is required"
            })
        ...
        if not periods:
            return jsonify({
                "success": False,
                "message": "No periods found in weights_data"
            })
        ...
        has_period_dates = any(
            'period' in p or ('train_start' in p and 'test_start' in p) or ('test_start' in p and 'test_end' in p)
            for p in periods
        )
        ...
        def calculate_advanced_metrics(daily_returns_list, benchmark_returns_list=None):
            ...
            if benchmark_returns_list:
                ...
                metrics['information_ratio'] = float((excess_mean / excess_std) * np.sqrt(ann_factor)) if excess_std > 0 else 0
        ...
        if backtest_mode == 'fixed':
            ...
            for period in periods:
                if period.get(method):
                    ...
                    print(f"[DEBUG] Found period with method '{method}': {len(selected_period.get('weights', []))} weights")
                    break
            ...
            print(f"[DEBUG] Method '{method}' - First 5 weights: {weights[:5] if len(weights) >= 5 else weights}")
            ...
            try:
                ...
                bench_df = D.features([bench_code], ['$close/$close(1)-1'], start_time=start_date, end_time=end_date)
                benchmark_returns = []
            ...
            print(f"[BACKTEST] Fixed mode - Combined expression (first 200 chars): {combined_expression[:200]}...")
            print(f"[BACKTEST] Fixed mode - Number of weighted terms: {len(weighted_terms)}")
            period_info = {
                'mode': 'fixed',
                'weights': weights,
            ...
            for idx, period in enumerate(periods):
                ...
                if 'period' in period and isinstance(period['period'], str):
                    ...
                    if len(parts) == 2:
                        try:
                            period_start = datetime.strptime(parts[0].strip(), '%Y-%m-%d')
                ...
                # Format 2: "period" field as dict with start/end
                    ...
                    if 'start' in p_dict and 'end' in p_dict:
                        try:
                            period_start = datetime.strptime(p_dict['start'], '%Y-%m-%d')
                ...
                # Format 3: train/test split - use test period
                    ...
                    try:
                        period_start = datetime.strptime(period['test_start'], '%Y-%m-%d')
                ...
                if not weights or not factor_names or len(weights) != len(factor_names):
                    print(f"[WARNING] Period {idx} has invalid weights/factors, skipping")
                    continue
                ...
                print(f"[INFO] ✓ Period {idx}: {period_start.date()} to {period_end.date()} - {len(weights)} factors")
            ...
            print(f"[BACKTEST] Dynamic mode: {len(period_weights)} periods covering {start_date} to {end_date}")
        ...
        try:
            if backtest_mode == 'fixed':
                ...
                print(f"[BACKTEST] Running backtest: {start_date} to {end_date}, market={market}")
                print(f"[BACKTEST] Expression hash: {hash(combined_expression)}")  # To detect if expression changes
                ...
                print(f"[BACKTEST] Running {len(period_weights)} period-specific backtests...")
                ...
                for p_idx, period in enumerate(period_weights):
                    ...
                    weighted_terms = []
                    for w, factor_expr in zip(period['weights'], period['factor_names']):
                        if abs(w) > 1e-10:
                            weighted_terms.append(f"({w})*({factor_expr})")
                    
                    if not weighted_terms:
                        print(f"[WARNING] Period {p_idx} has all zero weights, skipping")
                        continue
                    
                    period_expression = " + ".join(weighted_terms)
                    
                    print(f"[BACKTEST] Period {p_idx+1}/{len(period_weights)}: {period_start_str} to {period_end_str}")
                    ...
                    try:
                        ...
                        if analysis_df is not None and len(analysis_df) > 0:
                            try:
                                level0_values = analysis_df.index.get_level_values(0).unique()
                                pure_metrics = analysis_df.loc[level0_values[0]]
                                
                                period_metrics['annualized_return'] = float(pure_metrics.loc['annualized_return']) if 'annualized_return' in pure_metrics.index else 0
                                period_metrics['information_ratio'] = float(pure_metrics.loc['information_ratio']) if 'information_ratio' in pure_metrics.index else 0
                                period_metrics['max_drawdown'] = float(pure_metrics.loc['max_drawdown']) if 'max_drawdown' in pure_metrics.index else 0
                            except:
                                pass
                        ...
                        all_period_results.append({
                            ...
                            'expression': period_expression[:100] + '...' if len(period_expression) > 100 else period_expression
                        })
                ...
                if not all_daily_returns:
                    return jsonify({
                        "success": False,
                        "message": "All period backtests failed"
                    })
                ...
                aggregate_metrics = calculate_advanced_metrics(all_daily_returns, benchmark_returns)
                
                print(f"[BACKTEST] Dynamic mode complete: {len(all_period_results)} periods, {len(all_daily_returns)} trading days")
