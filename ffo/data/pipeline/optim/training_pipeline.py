...
class UnifiedOptimizationPipeline:
    ...
    def __init__(
        ...
        self.period_data_cache = {}  # Key: (period_start, period_end), Value: loaded data
        
        os.makedirs(model_save_path, exist_ok=True)
        logger.info(f"Pipeline initialized for {instruments} market with caching enabled")
    ...
    def run_fixed_period_optimization(
        ...
        if include_baseline:
            ...
            logger.info(f"Baseline result status: {baseline_result.get('status', 'UNKNOWN')}")
            if baseline_result.get('status') == 'success':
                ...
                period_result.baseline = OptimizationResult(
                    ...
                    status='success',
                    period={'start': start_date, 'end': end_date},
                    weights=baseline_result['weights'].tolist() if hasattr(baseline_result['weights'], 'tolist') else list(baseline_result['weights']),
                ...
                logger.error(f"Baseline failed: {baseline_result.get('message', 'Unknown error')}")
        ...
        if 'lasso' in methods:
            ...
            if lasso_result.get('status') == 'success':
                ...
                logger.error(f"LASSO failed: {lasso_result.get('message', 'Unknown error')}")
        ...
        if 'ic_optimization' in methods:
            ...
            logger.info(f"IC Optimization result status: {ic_result.get('status', 'UNKNOWN')}")
            if ic_result.get('status') == 'success':
                ...
                period_result.ic_optimization = OptimizationResult(
                    ...
                    metrics={
                        ...
                        'num_factors_used': int(np.sum(np.array(ic_result['weights']) > 0))
                    },
                    message='IC optimization computed'
                )
                logger.info(f"IC Optimization set: {period_result.ic_optimization is not None}")
            else:
                logger.error(f"IC Optimization failed: {ic_result.get('message', 'Unknown error')}")
    ...
    def run_dynamic_period_optimization(
        ...
        if lookback_window is not None:
            ...
            if method_configs:
                for method in methods:
                    if method in method_configs and 'lookback_window' in method_configs[method]:
            ...
            if user_provided_T is not None:
                ...
                if 'lasso' in methods and 'lasso' in default_configs:
                    global_lookback = default_configs['lasso'].get('lookback_window', 60)
                elif 'ic_optimization' in methods and 'ic_optimization' in default_configs:
                    global_lookback = default_configs['ic_optimization'].get('lookback_window', 60)
        ...
        for period_idx, period_config in enumerate(periods_config):
            ...
            if 'test_start' in period_config:
                ...
                if 'train_start' not in period_config or 'train_end' not in period_config:
                    from datetime import datetime, timedelta
                    ...
                    training_start_dt = training_end_dt - timedelta(days=est_calendar_days)
            ...
            logger.info(f"Period {period_idx + 1}: Train {period_start} to {period_end}, Test {test_start} to {test_end}")
    ...
    def validate_periods_continuity(periods_config: List[Dict]) -> Dict:
        ...
        for i in range(len(sorted_periods) - 1):
            try:
                ...
                next_start = datetime.strptime(sorted_periods[i+1]['start'], '%Y-%m-%d')
    ...
    def _compute_baseline_fixed(self, factor_expressions: List[str], start_date: str, end_date: str) -> Dict:
    ...
    def _create_summary(self, period_results: List[PeriodResults]) -> Dict:
        ...
        return {
            ...
            'successful_periods': len([p for p in period_results if p.baseline or p.lasso or p.ic_optimization])
        }
