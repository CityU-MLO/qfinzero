...
Enhance three high-priority backend capabilities across UPQ, ESP, and the Dashboard:

1. **UPQ Data Freshness** — New Rust endpoint returning timestamps, counts, and partition info
2. **ESP Query Enhancement** — Keyword/publisher filtering, statistics API, export, calendar coverage
...
## 3. ESP Query Enhancement

### 3.1 News Search with Keyword + Publisher Filtering

**New endpoint**: `POST /esp/news/search`
...
// Response: same PaginatedResponse as /esp/events/query
```
...
**New endpoint**: `GET /esp/news/stats?days=7`
...
**New endpoints**:
```
GET /esp/news/export?tickers=AAPL&start=2025-01-01&end=2025-01-03&format=jsonl
GET /esp/news/export?tickers=AAPL&start=2025-01-01&end=2025-01-03&format=csv
GET /esp/calendar/earnings/export?start=2025-01-01&end=2025-01-31&format=csv
GET /esp/calendar/economic/export?start=2025-01-01&end=2025-01-31&format=csv
```
...
**New endpoint**: `GET /esp/calendar/coverage`
...
### 3.5 ESP Freshness Endpoint

**New endpoint**: `GET /esp/health/freshness`

Extends existing `/esp/health` data_freshness into the standardized schema. Includes `daily_counts`, `top_tickers`, and `duplicate_rate` from the stats pipeline.

### Files Changed

```
infra/esp/routes/news.py            # New POST /esp/news/search
infra/esp/routes/stats.py           # New file: GET /esp/news/stats
infra/esp/routes/export.py          # New file: export endpoints
infra/esp/routes/calendar.py        # New GET /esp/calendar/coverage
infra/esp/routes/health.py          # New GET /esp/health/freshness
infra/esp/services/data_sources.py  # MongoNewsSource: keyword/publisher query support
infra/esp/models.py                 # New request/response models
infra/esp/main.py                   # Register new routers
clients/esp/client.py               # New methods: search, stats, export, coverage, freshness
```
...
```json
{
  "services": {
    ...
    "esp": {"status": "up", "metrics": {...}},
    "upq": {"status": "up", "metrics": {...}}
  },
  "freshness": {
    "upq": {"sources": {"stock_minute": {...}, ...}},
    "esp": {"sources": {"news": {...}, "earnings": {...}, ...}}
  }
}
```
...
## 5. ESP Sanity Check API (Minimal)

**New endpoint**: `GET /esp/admin/sanity`
...
```
infra/esp/routes/admin.py   # New file: GET /esp/admin/sanity
infra/esp/main.py           # Register admin router
```
...
| `clients/esp/client.py` | `search_news()`, `news_stats()`, `export_news()`, `export_earnings()`, `export_economic()`, `calendar_coverage()`, `freshness()`, `sanity_check()` |
