"""
ESP Client — Python client for the News Pushing Pipeline REST API.
...
Usage:
    ...
    with ESPClient() as esp:
        ...
        body = esp.news_body("some_article_id")
"""

import requests
from typing import Optional

from qfinzero.config import ESP_URL
...
class ESPClient:
    ...
    def _handle(self, resp: requests.Response):
        try:
            ...
            raise ESPError(f"Non-JSON response: {resp.status_code}", resp.status_code)
        if resp.status_code >= 400:
            msg = data.get("message", str(data))
            code = data.get("code", "unknown")
            raise ESPError(msg, resp.status_code, code)
        return data
    ...
    def query_events(
        ...
        return self._post("/esp/events/query", body)

    def get_event(self, event_id: str) -> dict:
        """Get a single event by its event_id."""
        return self._get(f"/esp/events/{event_id}")
    ...
    def stream(
        ...
        if now_utc:
            body["now_utc"] = now_utc
        return self._post("/esp/events/stream", body)
    ...
    def next_triggers(
        ...
        if now_utc:
            body["now_utc"] = now_utc
        return self._post("/esp/triggers/next", body)
    ...
    def econ_calendar(
        ...
        if now_utc:
            body["now_utc"] = now_utc
        return self._post("/esp/calendar/econ", body)
    ...
    def earnings_calendar(
        ...
        if now_utc:
            body["now_utc"] = now_utc
        return self._post("/esp/calendar/earnings", body)
    ...
    def news_body(self, news_id: str) -> dict:
        ...
        """
        return self._get(f"/esp/news/{news_id}/body")
    ...
    def earnings_export(
        ...
        resp = self._session.get(
            f"{self.base_url}/esp/calendar/earnings/export",
            params=params,
            timeout=self.timeout,
            stream=True,
        )
    ...
    def economic_export(
        ...
        resp = self._session.get(
            f"{self.base_url}/esp/calendar/economic/export",
            params=params,
            timeout=self.timeout,
            stream=True,
        )
    ...
    def timeline(
        ...
        if now_utc:
            body["now_utc"] = now_utc
        return self._post("/esp/timeline", body)
